...
export function FeatureCard({ feature, className, ...props }: FeatureCardPorps) {
	...
	useEffect(() => {
		setIsMounted(true);
		// Generate pattern only on client-side to prevent SSR hydration mismatch
		setPattern(genRandomPattern());
	}, []);

	// Use deterministic pattern for SSR
	const ssrPattern = [
		[8, 2], [9, 4], [7, 1], [10, 5], [8, 3]
	];

	const p = isMounted ? pattern : ssrPattern;
...
// Wrapper component for easy section integration
		...
		<div className={cn('grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6', className)} {...props}>
			{features.map((feature, index) => (
				<FeatureCard
					key={feature.title}
					feature={feature}
					className="border border-border bg-card rounded-xl hover:shadow-lg transition-all duration-300"
				/>
			))}
		</div>
	);
