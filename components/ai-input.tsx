...
export function MorphPanel({ userLocation, isMobileFullScreen, onClose }: { userLocation?: 'main' | 'bridvia-connect', isMobileFullScreen?: boolean, onClose?: () => void }) {
  ...
  const handleSuccess = React.useCallback(async (userMessage: string) => {
    ...
    try {
      ...
      // Basic cleanup for any remaining artifacts (backend handles main filtering)
      ...
      // Fallback response with basic cleanup
      let fallbackResponse = `I understand you're asking about "${userMessage.substring(0, 50)}${userMessage.length > 50 ? '...' : ''}". I apologize, but I'm currently unable to process your request. Please contact us at info@bridvia.com for assistance.`
      
      // Basic cleanup for fallback responses
      fallbackResponse = fallbackResponse.replace(/\s+/g, ' ').trim();
